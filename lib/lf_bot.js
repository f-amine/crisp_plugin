...
const qaPrompt = `
    ...
    - answer using the language from the question 
...
const buttonChoices = {
  id: "test",
  text: "The response was generated by Lightfunnels trained AI assistant",
  choices: [
    {
      value: "Continue speaking with AI",
      label: "Continue speaking with AI",
      icon: "🤖",
      selected: false,
    },
    {
      value: "Escelate to the support team",
      label: "Escelate to the support team",
      icon: "👨",
      selected: false,
    },
  ],
...
class LightFunnelsBot {
  ...
  //       } catch (error) {
  //         console.error(`Filed to get messages for conversation ${sessionId}:`, error);
  //       }
  //     }
  //   } catch (error) {
  ...
  _events() {
    ...
    const resetTimeout = (conversationId) => {
      ...
      const timeout = setTimeout(
        ...
        2 * 60 * 1000,
      ); //2 minutes for now (testing purposes)
    ...
    this.crispClient.on("message:send", async (event) => {
      try {
        ...
        const chatHistory = recentMessages.reduce((acc, message) => {
          if (typeof message.content === "string") {
            if (message.user.nickname === "Lightfunnels-AI") {
              acc.push(new AIMessage(message.content));
            } else {
              acc.push(new HumanMessage(message.content));
            }
          }
          return acc;
        }, []);
        ...
        if (
          result.answer.toLowerCase().includes("@yousra") ||
          result.answer
            .toLowerCase()
            .includes("contact lightfunnels support") ||
          result.answer.toLowerCase().includes("i am not sure") ||
          ...
          self.crispClient.website
            .sendMessageInConversation(event.website_id, event.session_id, {
              ...
              from: "operator",
              origin: self._pluginUrn,
              content: "@Yousra @Hafsa Please handle this conversation",
              user: {
                type: "participant",
                nickname: "Lightfunnels-AI",
                avatar:
    ...
    this.crispClient.on("message:updated", (event) => {
      console.log('Got "message:updated" event:', event.content.choices);
      const conversationId = `${event.website_id}-${event.session_id}`;
      if (event.content.choices[1].selected === true) {
        self.conversationStates.set(conversationId, "escalated");
        self.crispClient.website
          .sendMessageInConversation(event.website_id, event.session_id, {
            type: "text",
            ...
            origin: self._pluginUrn,
            content:
              "I'm sorry I couldn't help with your inquiry, I will pass the conversation to the lightfunnels support team",
            user: {
              type: "participant",
              nickname: "Lightfunnels-AI",
              avatar:

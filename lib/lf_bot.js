...
const qaPrompt = `
    You are a customer support agent for Lightfunnels and you are helping a user with an inquiry. Be 100% sure to follow these guidelines:
    
    Most importantly:
    - Answer the question **only** using the provided context below.
    - If you are not really confident about the answer or the context is not relevant to the query, your response **must** include the string '@Yousra'!!
    - **Do not** come up with answers that are not in the context. Instead, include '@Yousra' in the response.
    - Think step by step before providing a helpful and concise answer.
    - If an image is relevant, make sure to include it in the answer in markdown format (with its link), it should be in markdown format: ![](link here)
    - Also make sure to include the URL source in the response if it exists in the context.

    **Important:** If the context does not contain the answer or you are not sure about the answer, respond with '@Yousra'.
...
class LightFunnelsBot {
  ...
  //   var websiteId = '2fb540f1-8080-47f2-a548-f7a72a0f11bc';
  
  //   try {
  //     const conversations = await this.crispClient.website.listConversations(websiteId);
  ...
  //       try {
  //         const messages = await this.crispClient.website.getMessagesInConversation(websiteId, sessionId);
  //         const markdownContent = formatConversationToMarkdown(messages);
          
  //         const fileName = `conversation_${sessionId}.md`;
  ...
  //         console.log(`Conversation ${sessionId} saved to ${filePath}`);
  //       } catch (error) {
  //         console.error(`Failed to get messages for conversation ${sessionId}:`, error);
  //       }
  //     }
  //   } catch (error) {
  ...
  _events() {
    ...
    const resetTimeout = (conversationId) => {
      ...
      }, 2 * 60 * 1000); //2 minutes for now (testing purposes)
    ...
    this.crispClient.on("message:received", async (event) => {
      try {
        ...
        if (result.answer.toLowerCase().includes("@yousra") || result.answer.toLowerCase().includes("contact lightfunnels support") || result.answer.toLowerCase().includes('i am not sure') || result.answer.toLowerCase().includes('i don\'t know') ) {
          ...
          self.crispClient.website
            .sendMessageInConversation(event.website_id, event.session_id, {
              ...
              content:
                "I'm sorry for the inconvenience but as an ai assitant I'm not confident enough to answer your inquery.\n The conversation has been escalated to a human agent. Please wait for their response.",
              user: {
                type: "participant",
...
function formatConversationToMarkdown(messages) {
  return messages.map(message => {
    const user = message.user.nickname || message.user.user_id;
